...
import { SiteFooter, SiteHeader, PageHero, TagList, ProjectVisual } from "@/components/site-chrome";
        ...
        <section className="py-20 lg:py-28">
          <div className="mx-auto grid max-w-7xl gap-14 px-5 lg:grid-cols-[1.4fr_0.6fr] lg:px-8">
            <div>
              ...
              <div className="relative mt-6 aspect-[16/9] overflow-hidden bg-surface-strong">
                {!project.image && <div className="grid-lines absolute inset-0 opacity-40" />}
                <ProjectVisual visual={project.visual} icon={project.icon} image={project.image} alt={project.title} />
              </div>

              <h2 className="mt-14 text-xs font-semibold uppercase text-primary">Highlights</h2>
              <ul className="mt-6 grid gap-3 sm:grid-cols-2">
                {project.highlights.map((item) => (
                  <li key={item} className="flex items-start gap-3 border border-line bg-surface p-4 text-sm leading-6 text-muted-foreground">
