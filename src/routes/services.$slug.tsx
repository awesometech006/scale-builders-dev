...
export const Route = createFileRoute("/services/$slug")({
  ...
  head: ({ loaderData }) => {
    if (!loaderData) {
      return {
        meta: [
          { title: "Service not found — Technologics" },
      ...
      <main>
        ...
        <section className="py-20 lg:py-28">
          <div className="mx-auto grid max-w-7xl gap-14 px-5 lg:grid-cols-[1.4fr_0.6fr] lg:px-8">
            <div>
              ...
              <ul className="mt-6 border-t border-line">
                {service.includes.map((item) => (
                  <li
                    key={item}
                    className="flex items-start gap-3 border-b border-line py-4 text-sm leading-6"
                  >
              ...
              <h2 className="mt-14 text-xs font-semibold uppercase text-primary">
                Typical process
              </h2>
              <ol className="mt-6 grid gap-4">
                {service.process.map((step, index) => (
                  <li
                    key={step}
                    className="grid gap-3 border border-line bg-surface p-5 sm:grid-cols-[48px_1fr]"
                  >
            ...
            <aside>
              <h2 className="text-xs font-semibold uppercase text-primary">Awesome Tech</h2>
              <div className="mt-6">
                <TagList items={service.tech} />
              </div>
              <div className="mt-10 border border-line bg-surface p-6">
                <h3 className="text-lg font-semibold">
                  Planning {service.title.toLowerCase()} work?
                </h3>
