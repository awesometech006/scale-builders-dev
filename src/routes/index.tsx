...
import { SiteFooter, SiteHeader, ProjectVisual } from "@/components/site-chrome";
...
function Index() {
  ...
  const submitForm = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const response = await fetch("https://formsubmit.co/ajax/technologics@gmail.com", {
        method: "POST",
    ...
    >
      <div className="relative aspect-[4/3] overflow-hidden bg-surface-strong">
        ...
        <div className="absolute inset-0 transition-transform duration-500 group-hover:-translate-y-1">
          <ProjectVisual visual={visual} icon={icon} image={project.image} alt={title} />
        </div>
      </div>
